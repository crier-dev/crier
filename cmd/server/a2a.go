...
func (h *a2aHandler) handle(w http.ResponseWriter, r *http.Request) {
	...
	default:
		h.writeRPC(w, a2a.ErrorResponse(req.ID, a2a.NewRPCError(a2a.CodeMethodNotFound, fmt.Sprintf(
			"Method not found: this binding serves %s, %s, %s, %s, %s and %s; the push-notification-config methods and GetExtendedAgentCard are later rows (INT-A2A-005/006) and are not registered",
			a2a.MethodSendMessage, a2a.MethodSendStreamingMessage, a2a.MethodGetTask,
			a2a.MethodListTasks, a2a.MethodCancelTask, a2a.MethodSubscribeToTask))))
	}
}
