...
INT-A2A-005/006 (the push-notification configs and the extended card — NOT shipped)
...
| `POST /a2a` — a JSON-RPC 2.0 endpoint accepting `GetTask`, `ListTasks`, `CancelTask`, `SubscribeToTask`, `SendMessage` and `SendStreamingMessage`, with `Content-Type: text/event-stream` for the two streaming ones | §9.4 core methods | `CR_A2A_ENABLED=true` **and** the tenant's row opted in | **SHIPPED (INT-A2A-003 for the sends §5.4, INT-A2A-004 for the task lifecycle §5.5)** |
| The push-notification-config methods and `GetExtendedAgentCard` on the same endpoint | §9.4 core methods | `CR_A2A_ENABLED=true` | FUTURE (INT-A2A-005/006) — the endpoint is registered, and each of these methods answers `-32601 MethodNotFoundError` naming the row that lands it |
...
| `-32601` | `MethodNotFoundError` | any method but the six this series serves (`SendMessage`, `SendStreamingMessage`, `GetTask`, `ListTasks`, `CancelTask`, `SubscribeToTask`), naming the row that lands the rest |
...
| `-32004` | `UnsupportedOperationError` | `message.taskId` (task continuation, §5.5.3), a `SubscribeToTask` aimed at a terminal task (§9.4.6), or a task-lifecycle operation on a relay whose inbox backend cannot answer the read or the close it needs |
...
- **No task lifecycle operations** — INT-A2A-004 landed them (§5.5). `GetTask`, `ListTasks`,
  `CancelTask` and `SubscribeToTask` are served; every push-notification method and
  `GetExtendedAgentCard` still answers `-32601` (INT-A2A-005/006).
...
- **No artifacts.** `Task.artifacts` is empty (and omitted unless asked for); crier stores no artifact
  for a task.
- **No push-notification configs and no extended card** — INT-A2A-005/006, still `-32601`.
...
WebSocket relay round trip with the option ON). The task lifecycle is gated by
`internal/a2a/task_test.go` (the mapping table of §5.5.2 row by row, the parameters, the ordering and
the cursor, the refusal table) and `cmd/server/a2a_lifecycle_test.go` (the route booted for real: the
lifecycle transcript — undelivered → leased → acked → expired → canceled — the terminal-state
refusals, the paginated listing, the swept-task arm driven through crier's own sweep, a live
`SubscribeToTask`, the per-agent gate, and the switch-off 404). Only the two INT-A2A-003 test files
whose subject this row's method surface moved were amended (`a2a_jsonrpc_test.go`: the method that
still answers `MethodNotFoundError` is a push-notification one now, and a `taskId` naming no task is
covered as the `-32001` §5.5.3 makes it); the non-regression gate and every crier route test are
untouched.
...
Not shipped by that row, and still absent under both switch positions: the push-notification configs
and the extended card (INT-A2A-005/006 — the task lifecycle landed as INT-A2A-004, §8.4), and any
change to the relay's WebSocket path (deliberate: the stream is an adapter OVER it).
...
Not shipped by that row: the push-notification configs and the extended card (INT-A2A-005/006), any
storage change (the three capabilities read and close the SAME rows), and any change to the routing,
auth, guard, federation, webhook or mesh behaviour of the delivery path.
