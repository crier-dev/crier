...
The seven files below are crier's **normative design authority**: they define the contracts that
...
| [`A2A-OPTION.md`](A2A-OPTION.md) | A2A interoperability as an OPT-IN extra (INT-A2A-001/002/003/005): the binding decision (JSON-RPC 2.0 over HTTP + SSE implemented; gRPC and HTTP+JSON/REST declared MAY, not built), the A2A ⇄ crier object mapping, the two-half gate (`CR_A2A_ENABLED` default false + the optional per-agent `a2a` block), the route surface (two routes while the switch is on: the Agent Card discovery route, §5.3, and the JSON-RPC binding `POST /a2a`, §5.4, whose methods include the four push-notification configuration operations of §5.5 — a view over the agent's own webhook config, with the §3.3.4 `PushNotificationNotSupportedError` for an agent that has no push channel), the JSON-RPC binding's request/part/error/streaming contracts and the one stated deviation from the spec's blocking default, the non-regression contract, and the binding non-goals. | DRAFT v3 · 2026-09-25 · INT-A2A-001 (the gate), INT-A2A-002 (Agent Card, SHIPPED), INT-A2A-003 (JSON-RPC binding, SHIPPED), INT-A2A-005 (push-notification configuration, SHIPPED), INT-A2A-004/006 (the remaining surfaces, not yet built) |
...
| [`NAMESPACES.md`](NAMESPACES.md) | The namespace (realm) dimension (CR-FEAT-029): the WAMP realm semantics it steals from (and the four it deliberately does not), the default-namespace-as-`""` canonical mapping that keeps a single-namespace deployment byte-identical, the per-realm policy axes (auth posture, rate limits, guard settings, retention) with their inheritance rules, where a realm is decided on each lane (registration / delivery / transfer / relay publish+subscribe), the config document and its fail-closed validation, the wire contract, migration 008's NULL-means-default storage, the non-regression and acceptance evidence tables, and the honest non-goals (mesh lane not realm-scoped, no cross-realm bridging, no live realm move). | Implemented · 2026-09-25 · CR-FEAT-029 (source: external review DISPATCH · CRI-001 by Carter) |
...
Spec inventory reconciled 2026-09-25 (A2A-OPTION.md added — INT-A2A-001; DETECTION.md added — CR-FEAT-030;
NAMESPACES.md added — CR-FEAT-029).
