...
| [`A2A-OPTION.md`](A2A-OPTION.md) | A2A interoperability as an OPT-IN extra (INT-A2A-001/002/003/004): the binding decision (JSON-RPC 2.0 over HTTP + SSE implemented; gRPC and HTTP+JSON/REST declared MAY, not built), the A2A ⇄ crier object mapping, the two-half gate (`CR_A2A_ENABLED` default false + the optional per-agent `a2a` block), the route surface (two routes while the switch is on: the Agent Card discovery route, §5.3, and the JSON-RPC binding `POST /a2a`, §5.4), the JSON-RPC binding's request/part/error/streaming contracts and the one stated deviation from the spec's blocking default, the task lifecycle (`GetTask` / `ListTasks` / `CancelTask` / `SubscribeToTask` mapped onto the inbox entry, §5.5 — including the state-mapping table §5.5.2 and what crier cannot report), the non-regression contract, and the binding non-goals. | DRAFT v2 · 2026-09-25 · INT-A2A-001 (the gate), INT-A2A-002 (Agent Card, SHIPPED), INT-A2A-003 (JSON-RPC binding, SHIPPED), INT-A2A-004 (task lifecycle, SHIPPED — §5.5), INT-A2A-005/006 (the remaining surfaces, not yet built) |
...
Spec inventory reconciled 2026-09-25 (A2A-OPTION.md re-reconciled for INT-A2A-004 — the task lifecycle's §5.5, its state-mapping table §5.5.2, and the three optional store capabilities §5.5.5; the route surface is unchanged at two routes).
