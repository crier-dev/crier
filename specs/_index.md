...
| [`A2A-OPTION.md`](A2A-OPTION.md) | A2A interoperability as an OPT-IN extra (INT-A2A-001): the binding decision (JSON-RPC 2.0 over HTTP + SSE implemented; gRPC and HTTP+JSON/REST declared MAY, not built), the A2A ⇄ crier object mapping, the two-half gate (`CR_A2A_ENABLED` default false + the optional per-agent `a2a` block), the route surface (none registered today; the future surfaces are enumerated), the non-regression contract, and the binding non-goals. | DRAFT v1 · 2026-09-25 · INT-A2A-001 (the gate: switch + opt-in + non-regression proof), INT-A2A-002..006 (the surfaces, not yet built) |
...
| [`NAMESPACES.md`](NAMESPACES.md) | The namespace (realm) dimension (CR-FEAT-029): the WAMP realm semantics it steals from (and the four it deliberately does not), the default-namespace-as-`""` canonical mapping that keeps a single-namespace deployment byte-identical, the per-realm policy axes (auth posture, rate limits, guard settings, retention) with their inheritance rules, where a realm is decided on each lane (registration / delivery / transfer / relay publish+subscribe), the config document and its fail-closed validation, the wire contract, migration 007's NULL-means-default storage, the non-regression and acceptance evidence tables, and the honest non-goals (mesh lane not realm-scoped, no cross-realm bridging, no live realm move). | Implemented · 2026-09-25 · CR-FEAT-029 (source: external review DISPATCH · CRI-001 by Carter) |
