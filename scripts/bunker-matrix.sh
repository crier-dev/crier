...
#                         [--sink http://100.97.236.14:19002] [--skip-build]
set -uo pipefail
...
CR_ENV_FILE=/tmp/mx-guard-on.env bash "$REPO/scripts/bunker-deploy.sh" --agent "$AGENT" --host "$HOST" --port "$PORT" $EXTRA_DEPLOY_ARGS
...
bash "$REPO/scripts/bunker-deploy.sh" --agent "$AGENT" --host "$HOST" --port "$PORT" --skip-build
register guard-off ''
...
CR_ENV_FILE=/tmp/mx-fail-closed.env bash "$REPO/scripts/bunker-deploy.sh" --agent "$AGENT" --host "$HOST" --port "$PORT" --skip-build
...
if [[ -n "$SINK" ]]; then
  ...
  printf 'CR_REQUIRE_AGENT_SIG=false\n' > /tmp/mx-blocking.env
  CR_ENV_FILE=/tmp/mx-blocking.env bash "$REPO/scripts/bunker-deploy.sh" --agent "$AGENT" --host "$HOST" --port "$PORT" --skip-build
  register blocking ''
