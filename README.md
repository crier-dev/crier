...
| `CR_A2A_ENABLED` | `false` | Opt-in: A2A (agent-to-agent protocol) interoperability — INT-A2A-001/002/003/005, [`specs/A2A-OPTION.md`](specs/A2A-OPTION.md). **Default off, and A2A is an extra rather than first-class support**: with the flag unset nothing A2A-related is registered, and every existing route, response body, auth requirement and storage path behaves exactly as it did before the option existed. The flag is also only HALF the gate — an agent takes part in A2A only if it opted in as well, via the optional `a2a` object on `POST /agents` / `PATCH /agents/{id}` (`{"a2a":{"enabled":true}}`, strictly decoded, absent by default). With the flag set crier publishes exactly TWO A2A surfaces, and nothing else: (1) `GET /.well-known/agent-card.json?agent_id=<id>` serves the [A2A](https://a2a-protocol.org) Agent Card projected from that agent's registry row (INT-A2A-002) — `404` for an id that is not an opted-in row, `400` when the request names no agent, `Cache-Control: private` + `ETag` for conditional GETs; (2) `POST /a2a` is the A2A JSON-RPC 2.0 binding (INT-A2A-003, INT-A2A-005) serving `SendMessage` (a `Task` or a direct `Message`) and `SendStreamingMessage` (`text/event-stream`, carrying the task's own inbox lifecycle plus everything published on the task's relay topic) — both reachable only for an agent whose row opted in, both translating into crier's EXISTING delivery path (`POST /agents/{id}/inbox`, its own handler) rather than a second delivery engine, and both unregistered (answering the router's own `404`) when the flag is unset — plus the four push-notification configuration methods (INT-A2A-005): `CreateTaskPushNotificationConfig`, `GetTaskPushNotificationConfig`, `ListTaskPushNotificationConfigs` and `DeleteTaskPushNotificationConfig`, which are a VIEW over the agent's existing webhook config (no second store, no second delivery path: a write goes through `PATCH /agents/{id}`'s own handler, and the notification is an ordinary webhook delivery whose payload is the [A2A](https://a2a-protocol.org) `StreamResponse` envelope). Those four answer `PushNotificationNotSupportedError` (`-32003`) for an agent with no webhook — its Agent Card already says `capabilities.pushNotifications: false`. The task lifecycle operations (`GetTask`, `ListTasks`, `CancelTask`, `SubscribeToTask`) and the extended card land with INT-A2A-004/006: they answer `-32601 MethodNotFoundError` today. |
