...
func requireSilent(t *testing.T, conn *websocket.Conn, wait time.Duration) {
	t.Helper()
...
func TestMeshInboxNotifyRequiresTheOptIn(t *testing.T) {
	...
	requireSilent(t, conn, 300*time.Millisecond)
}
...
func TestMeshInboxNotifyForAnUnconnectedAgentIsFalse(t *testing.T) {
	...
	requireSilent(t, conn, 200*time.Millisecond)
}
...
func TestMeshInboundInboxNotifyIsIgnoredNotRefused(t *testing.T) {
	...
	requireSilent(t, conn, 500*time.Millisecond)
}
