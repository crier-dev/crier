...
// agent is not found.
func (s *Store) Deliver(agentID string, entry *InboxEntry) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) Retrieve(agentID string, leaseDuration time.Duration, maxMessages int) ([]*InboxEntry, string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) Ack(agentID, leaseID string, messageIDs []string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for _, entry := range queue {
		if idSet[entry.ID] {
			...
			if entry.LeaseID != leaseID {
				return fmt.Errorf("message %q is not leased under lease %q (current lease: %q)", entry.ID, leaseID, entry.LeaseID)
			}
		}
	}
	if found < len(messageIDs) {
		return fmt.Errorf("%d of %d message(s) not found", len(messageIDs)-found, len(messageIDs))
	}
...
// agent's inbox.
func (s *Store) Stats(agentID string) (queueDepth, leasedCount int, oldestAge time.Duration, err error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	if _, ok := s.agents[agentID]; !ok {
		return 0, 0, 0, fmt.Errorf("agent %q not found", agentID)
	}
...
// newLeaseID generates a random hex string for lease identification.
