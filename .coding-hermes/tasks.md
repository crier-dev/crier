...
> ⚠️ **BANE:** Crier has been stable for 15 ticks (~4 days of idle time across 7 days). 30 tasks completed. 78.6% coverage. All tests green. One DOC regression found and fixed this tick. CI is GitHub Actions (crier-dev/crier), latest run failed (idle tick #11, 27s — likely infra). Cooldown reversion continues (10th time — daemon restart overwrites fleet TOML config to 1800s). **Recommend disable project** — `PUT /api/v1/projects/crier {"Enabled":false}`.
