...
- [ ] **CI-012: Fix flaky Go 1.25 test in CI** (found 2026-07-20, never-done audit)
  - `cmd/server` test gets `signal: terminated` on Go 1.25 runner (1.26 passes)
  - Coverage + Integration jobs skipped because build job matrix failure blocks them
  - Likely `TestServerHealth` timeout — 1.25 runner is slower; server startup + health check + shutdown may exceed CI runner time
  - Options: increase test timeout, skip server test on 1.25, or use `t.Setenv` port zero consistently

- [ ] **PITFALL-003: Tighten gitleaks allowlist — secrets in markdown/docs go undetected** (found 2026-07-20, never-done audit)
  - `.gitleaks.toml` allowlists `*.md`, `specs/`, `docs/` (auto-generated by `gitreins init`)
  - This means API keys and secrets embedded in markdown docs, spec files, or documentation will NOT be caught by pre-commit secret scanning
  - Fix: remove `.*\\.md$`, `specs/`, `docs/` from allowlist paths; add only specific false-positive files

- [ ] **QUALITY-002: Fix .vfs/ gitignore — edges.jsonl must be tracked** (found 2026-07-20, never-done audit)
  - `.gitignore` line 33 blocks entire `.vfs/` directory — this prevents Hilo's `edges.jsonl` from being committed
  - Per hilo-usage skill: only `.vfs/graph/graph.db`, `.vfs/graph/graph.db.wal`, `.vfs/graph/graph.duckdb`, `.vfs/graph/.last_warm` should be gitignored
  - `edges.jsonl` and `.vfs/.dirty` marker MUST be tracked for cross-machine graph sync
  - Fix: replace `.vfs/` with the four specific cache file paths; `git add -f .vfs/graph/edges.jsonl`
